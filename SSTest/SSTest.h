...
#define SST_M_ASSERT_NEQ(x, y) { auto ass__a = (x); auto ass__b = (y); if(!(ass__a != ass__b)) __asserts__.push_back(SST::AssertFailure( std::string(#x " != " #y), SST::ToString(SST::ToType<decltype(ass__b)>(ass__a)) + " == " + SST::ToString(ass__b), __FILE__, __PRETTY_FUNCTION__, __LINE__)); }
...
#define SST_M_ASSERT_EQf(x, y) { auto ass__a = (x); auto ass__b = (y); if(!SST::AlmostEqual(float(ass__a), float(ass__b), 40)) __asserts__.push_back(SST::AssertFailure( std::string(#x " == " #y), SST::ToString(SST::ToType<decltype(ass__b)>(ass__a)) + " != " + SST::ToString(ass__b), __FILE__, __PRETTY_FUNCTION__, __LINE__)); }
